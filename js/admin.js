...
function getAdminItems() {
  ...
  const lostItems = [...getStoredArray(LOCAL_LOST_ITEMS_KEY), ...demoLostItems].map(item => ({
    ...item,
    type: "Lost",
    date: item.dateLost,
    adminId: createAdminId("Lost", item)
  }));
  const foundItems = [...getStoredArray(LOCAL_FOUND_ITEMS_KEY), ...demoFoundItems].map(item => ({
    ...item,
    type: "Found",
    date: item.dateFound,
